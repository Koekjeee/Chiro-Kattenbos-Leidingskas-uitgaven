// Logic for evenementen hub: financieel-only roles panel + filter event cards for non-financieel with event roles
(function(){
      ...
      ;['zomerbar','winterbar','schools-in-quiz'].forEach((name, idx)=>{
        ...
        cb.addEventListener('change', async ()=>{
          try {
            ...
            console.warn('Opslaan evenementenrol mislukt:', err);
  ...
  async function boot(){
    firebase.auth().onAuthStateChanged(async user =>{
      ...
      // Hide Evenementen nav if no access (redundant for index, but hub might be deep linked)
